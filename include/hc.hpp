...
struct projection_helper<T, 1>
{
    ...
    static result_type project(hector<T, 1>& now, int i) __CPU__ __HC__ {
#if __KALMAR_ACCELERATOR__ != 1
        now.cache.get_cpu_access(true);
#endif
        T *ptr = reinterpret_cast<T *>(now.cache.get() + i + now.offset + now.index_base[0]);
    ...
    static result_type project(const hector<T, 1>& now, int i) __CPU__ __HC__ {
#if __KALMAR_ACCELERATOR__ != 1
        now.cache.get_cpu_access(true);
#endif
        T *ptr = reinterpret_cast<T *>(now.cache.get() + i + now.offset + now.index_base[0]);
...
class hector {
 ...
 public:
  ...
  hector(hector& that) __CPU__ __HC__
    : cache(that.v_data.size(), that.v_data.data()), v_data(that.v_data), offset(that.offset){
  }
  hector(size_t size) : v_data(size), cache(size, nullptr) {}
  hector() : cache(0, nullptr) {}
  ~hector() __CPU__ __HC__ {}
  ...
  hector& operator=(const hector&& other) __CPU__ __HC__  = delete;
  hector& operator=(const hector& other) __CPU__ __HC__ = delete;

  typename projection_helper<T, N>::result_type
  operator[] (int i) const  __HC__ {
  ...
  {
    v_data.emplace_back(std::forward<Args>(args)...);
  }
  const T& operator[] (int i) const __CPU__  {
      return v_data[i];
  }
  T& operator[] (int i)  __CPU__  {
      return v_data[i];
  }
  size_t size(){ return v_data.size(); }
